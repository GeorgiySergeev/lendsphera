    ...
    <div className="flex h-10 items-center gap-1 border-b px-4 shrink-0 bg-muted/30 overflow-x-auto">
        ...
        <div key={variant.id} className="group flex items-center">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-5 w-5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <DropdownMenuContent>
              ...
              {variant.id !== 'default' && (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem className="text-destructive" onClick={() => handleDelete(variant.id)}>
                    Delete
                  </DropdownMenuItem>
                </>
              )}
      ...
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="sm" className="h-7 px-3 text-sm text-muted-foreground ml-2">
        ...
        <PopoverContent className="w-60 p-3" align="start">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button size="sm" className="h-8" onClick={handleCreateVariant}>Create</Button>
            </div>
          </div>
        </PopoverContent>
      </Popover>
