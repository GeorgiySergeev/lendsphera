...
export function NewComponentClient() {
  ...
  const hasChanges = useMemo(
    () =>
      ...
      html !== baselineRef.current.html ||
      css !== baselineRef.current.css,
  ...
  const handleSave = useCallback(async () => {
    ...
    if (html.trim().length < 10)
      return toast.error("HTML content must be at least 10 characters");
  ...
  }, [hasChanges, name, categoryId, html, css, createMutation, router]);
    ...
    <div className="flex h-full flex-col bg-background text-foreground">
      ...
      <header className="sticky top-0 z-10 flex h-14 shrink-0 items-center justify-between border-b px-4">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="font-medium text-sm whitespace-nowrap shrink-0">
            New Component
          </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center rounded-md border p-0.5">
                  <Button
                    variant="ghost"
                    size="icon"
                    className={`h-7 w-7 rounded-sm ${device === "mobile" ? "bg-muted" : ""}`}
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className={`h-7 w-7 rounded-sm ${device === "tablet" ? "bg-muted" : ""}`}
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className={`h-7 w-7 rounded-sm ${device === "desktop" ? "bg-muted" : ""}`}
          ...
          >
            ...
            <div className="flex-1 min-h-0 relative">
              <MonacoEditor
                ...
                onChange={
                  editorTab === "html" ? handleHtmlChange : (v) => setCss(v ?? "")
                }
                onMount={(_, monaco) => {
          ...
          {showTemplates && (
            <>
              ...
              <ResizablePanel
                ...
                minSize={15}
                maxSize={30}
                className="bg-muted/10 border-l flex flex-col"
              >
